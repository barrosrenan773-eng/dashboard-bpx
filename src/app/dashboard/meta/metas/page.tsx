...
function MetasContent() {
  ...
  const [metasLoja, setMetasLoja] = useState<Record<string, { meta: number; meta_conversao: number; meta_usuarios: number }>>({})
  const [loadingLojas, setLoadingLojas] = useState(true)
  const [editandoLoja, setEditandoLoja] = useState<string | null>(null)
  const [editLoja, setEditLoja] = useState('')
  const [editLojaConversao, setEditLojaConversao] = useState('')
  const [editLojaUsuarios, setEditLojaUsuarios] = useState('')
  ...
  const [editVendedorConversao, setEditVendedorConversao] = useState('')
  const [editVendedorTicket, setEditVendedorTicket] = useState('')
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const metasMap: Record<string, { meta: number; meta_conversao: number; meta_usuarios: number }> = {}
      metasArr.forEach((m: any) => {
        metasMap[m.vendedor] = { meta: m.meta || 0, meta_conversao: m.meta_conversao || 0, meta_usuarios: m.meta_usuarios || 0 }
      })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Adapta formato: { consultores: [{nome, deals, receita}] } → { vendedores: [{name, leads, revenue}] }
      const vendedores = (clintData?.consultores ?? []).map((c: any) => ({
        name: c.nome,
        leads: c.deals,
        revenue: c.receita,
      }))
      setClint({ vendedores, revenue: clintData?.receita ?? 0 })
      setMetasVendedor(Array.isArray(metasData) ? metasData : [])
      setLoadingVendedores(false)
    })
  }, [mes])

  async function salvarMetaLoja(loja: string, meta: number, meta_conversao: number, meta_usuarios: number) {
    await fetch('/api/metas-loja', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ loja, mes, meta, meta_conversao, meta_usuarios }),
    })
    setMetasLoja(prev => ({ ...prev, [loja]: { meta, meta_conversao, meta_usuarios } }))
    setEditandoLoja(null)
  }

  async function salvarMetaVendedor(vendedor: string, meta: number, metaLeads: number, metaConversao: number, metaTicket: number) {
    await fetch('/api/metas-vendedor', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ vendedor, mes, meta, meta_leads: metaLeads, meta_conversao: metaConversao, meta_ticket: metaTicket }),
    })
    setMetasVendedor(prev => {
      const exists = prev.find(m => m.vendedor === vendedor)
      if (exists) return prev.map(m => m.vendedor === vendedor ? { ...m, meta, meta_leads: metaLeads, meta_conversao: metaConversao, meta_ticket: metaTicket } : m)
      return [...prev, { vendedor, mes, meta, meta_leads: metaLeads, meta_conversao: metaConversao, meta_ticket: metaTicket }]
    })
    setEditandoVendedor(null)
  }

  const normalizarNome = (s: string) =>
    s.replace(/#\d*\s*/g, '').replace(/[#@!]/g, '').trim()
     .replace(/\b\w+/g, w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())

  const vendedores: any[] = (clint?.vendedores || []).map((v: any) => {
    const nomePadrao = normalizarNome(v.name ?? '')
    const m = metasVendedor.find(m => normalizarNome(m.vendedor) === nomePadrao)
    return { ...v, name: nomePadrao, meta: m?.meta || 0, metaLeads: m?.meta_leads || 0, metaConversao: m?.meta_conversao || 0, metaTicket: m?.meta_ticket || 0 }
  })

  const totalLojasMeta = LOJAS.reduce((s, l) => s + (metasLoja[l.key]?.meta || 0), 0)
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="p-6 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {LOJAS.map(l => {
              ...
              const m = metasLoja[l.key] || { meta: 0, meta_conversao: 0, meta_usuarios: 0 }
              const meta = m.meta
                  ...
                  {editandoLoja === l.key ? (
                    <div className="space-y-2">
                      <div className="flex items-center gap-1">
                        <input type="number" value={editLoja} onChange={e => setEditLoja(e.target.value)}
                          ...
                          placeholder="Meta Receita R$" autoFocus />
                      </div>
                      <div className="flex items-center gap-1">
                        <input type="number" value={editLojaConversao} onChange={e => setEditLojaConversao(e.target.value)}
                          className="w-28 bg-zinc-800 border border-zinc-600 text-white text-xs rounded px-2 py-1 focus:outline-none focus:border-blue-500"
                          placeholder="Meta Conv. %" step="0.1" />
                      </div>
                      <div className="flex items-center gap-1">
                        <input type="number" value={editLojaUsuarios} onChange={e => setEditLojaUsuarios(e.target.value)}
                          className="w-28 bg-zinc-800 border border-zinc-600 text-white text-xs rounded px-2 py-1 focus:outline-none focus:border-purple-500"
                          placeholder="Meta Usuários" />
                      </div>
                      <div className="flex gap-1 pt-1">
                        <button onClick={() => salvarMetaLoja(l.key, parseFloat(editLoja) || 0, parseFloat(editLojaConversao) || 0, parseInt(editLojaUsuarios) || 0)} className="text-emerald-400 hover:text-emerald-300 p-1">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <p className="text-zinc-500 text-xs">{meta > 0 ? `Meta receita: ${formatCurrency(meta)}` : 'Sem meta definida'}</p>
                        <button onClick={() => { setEditandoLoja(l.key); setEditLoja(String(meta || '')); setEditLojaConversao(String(m.meta_conversao || '')); setEditLojaUsuarios(String(m.meta_usuarios || '')) }}
                          className="text-zinc-500 hover:text-zinc-300 p-1 transition-colors">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                      </div>
                      {m.meta_conversao > 0 && <p className="text-zinc-600 text-xs">Meta conversão: {m.meta_conversao}%</p>}
                      {m.meta_usuarios > 0 && <p className="text-zinc-600 text-xs">Meta usuários: {formatNumber(m.meta_usuarios)}</p>}
                    </div>
                  )}
                </div>
              )
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            <h2 className="text-white font-bold text-lg">Consultores (CRM)</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-800">
                    {['Consultor', 'Receita', 'Meta R$', '% Meta', 'Leads', 'Meta Leads', '% Leads', 'Conversão', 'Meta Conv.', 'Ticket', 'Meta Ticket', ''].map(h => (
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {loadingVendedores ? (
                    ...
                    const ticket = v.leads > 0 ? v.revenue / v.leads : 0
                    const conversao = v.leads > 0 ? (v.leads / v.leads) * 100 : 0 // deals/leads — mas v.leads aqui é deals (metas page adapta assim)
                    return (
                      <tr key={v.name} className="hover:bg-zinc-800/30 transition-colors">
                        ...
                        <td className="py-3 px-3 text-zinc-300 text-xs">{ticket > 0 ? formatCurrency(ticket) : '—'}</td>
                        <td className="py-3 px-3">
                          {editandoVendedor === v.name ? (
                            <input type="number" value={editVendedorConversao} onChange={e => setEditVendedorConversao(e.target.value)}
                              className="w-16 bg-zinc-800 border border-zinc-600 text-white text-xs rounded px-2 py-1 focus:outline-none focus:border-blue-500"
                              placeholder="%" />
                          ) : (
                            <span className="text-zinc-300">{v.metaConversao > 0 ? `${v.metaConversao}%` : <span className="text-zinc-600 italic text-xs">—</span>}</span>
                          )}
                        </td>
                        <td className="py-3 px-3 text-zinc-300 text-xs">{ticket > 0 ? formatCurrency(ticket) : '—'}</td>
                        <td className="py-3 px-3">
                          {editandoVendedor === v.name ? (
                            <input type="number" value={editVendedorTicket} onChange={e => setEditVendedorTicket(e.target.value)}
                              className="w-20 bg-zinc-800 border border-zinc-600 text-white text-xs rounded px-2 py-1 focus:outline-none focus:border-orange-500"
                              placeholder="R$" />
                          ) : (
                            <span className="text-zinc-300">{v.metaTicket > 0 ? formatCurrency(v.metaTicket) : <span className="text-zinc-600 italic text-xs">—</span>}</span>
                          )}
                        </td>
                        <td className="py-3 px-3">
                          {editandoVendedor === v.name ? (
                            <div className="flex gap-1">
                              <button onClick={() => salvarMetaVendedor(v.name, parseFloat(editVendedor) || 0, parseInt(editVendedorLeads) || 0, parseFloat(editVendedorConversao) || 0, parseFloat(editVendedorTicket) || 0)} className="text-emerald-400 hover:text-emerald-300 p-1">
                                <Check className="w-3.5 h-3.5" />
                              </button>
                            ...
                            <button onClick={() => { setEditandoVendedor(v.name); setEditVendedor(String(v.meta || '')); setEditVendedorLeads(String(v.metaLeads || '')); setEditVendedorConversao(String(v.metaConversao || '')); setEditVendedorTicket(String(v.metaTicket || '')) }}
                              className="text-zinc-500 hover:text-zinc-300 p-1 transition-colors">
                              <Pencil className="w-3.5 h-3.5" />
                            </button>
                          )}
